# postali-api-rest-mcp

[MCP](https://modelcontextprotocol.io) server exposing the PostaliApiRest SDK as
two agent tools — `postali-api-rest_list` and `postali-api-rest_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/postali-api-rest-mcp)
make build
...
claude mcp add --scope user postali-api-rest \
  -- /absolute/path/to/postali-api-rest-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./postali-api-rest-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// postali-api-rest_list: first page of records
{ "entity": "state" }
{ "entity": "state", "query": { } }

// postali-api-rest_load: one record by id
{ "entity": "municipality", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/postali-api-rest-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user postali-api-rest \
     -- "$PWD"/dist/*/postali-api-rest-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `postali-api-rest_list` and `postali-api-rest_load` tools now appear
   in new sessions. Ask the agent to *"list state using postali-api-rest"*
   and it calls `postali-api-rest_list` with `{"entity":"state"}`.
...
### Call the `postali-api-rest_list` tool
...
### Call the `postali-api-rest_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `postali-api-rest_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `postali-api-rest_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./postali-api-rest-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"postali-api-rest_load","arguments":{"entity":"municipality","query":{"id":1}}}}'
```
