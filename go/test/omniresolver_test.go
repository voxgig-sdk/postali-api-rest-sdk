...
import (
	...
	omni "github.com/voxgig-sdk/postali-api-rest-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PostaliApiRestError); is {
				out := map[string]any{
					"name":    "PostaliApiRestError",
...
func omniCtx(arg any, client *sdk.PostaliApiRestSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PostaliApiRestSDK); is {
			client = live
