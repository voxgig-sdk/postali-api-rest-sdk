...
from postaliapirest_sdk.postaliapirest_types import (
    PostalCode,
    PostalCodeLoadMatch,
)
...
class PostalCodeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PostalCodeEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> PostalCode:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.PostalCode().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
