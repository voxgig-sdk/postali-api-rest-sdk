# PostaliApiRest SDK test runner
...
class PostaliApiRestTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PostaliApiRestTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PostaliApiRestTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PostaliApiRestTestRunner.getenv("POSTALI_API_REST_TEST_LIVE")
        override = PostaliApiRestTestRunner.getenv("POSTALI_API_REST_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PostaliApiRestTestRunner.getenv(key)
        ...
        explain = PostaliApiRestTestRunner.getenv("POSTALI_API_REST_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["POSTALI_API_REST_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if PostaliApiRestTestRunner._test_control is not None:
            return PostaliApiRestTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PostaliApiRestTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PostaliApiRestTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PostaliApiRestTestRunner.load_test_control()
...
def load_env_local():
    PostaliApiRestTestRunner.load_env_local()


def env_override(m):
    return PostaliApiRestTestRunner.env_override(m)


def entity_data(v):
    return PostaliApiRestTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PostaliApiRestTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PostaliApiRestTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PostaliApiRestTestRunner.load_test_control()


def live_client_options():
    return PostaliApiRestTestRunner.live_client_options()


def live_delay_ms():
    return PostaliApiRestTestRunner.live_delay_ms()
