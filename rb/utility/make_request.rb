...
module PostaliApiRestUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PostaliApiRestResponse.new({})
    ...
    if fetch_err
      ...
      response = PostaliApiRestResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PostaliApiRestResponse.new(fetched)
    else
