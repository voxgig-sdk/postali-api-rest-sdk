...
require_relative 'PostaliApiRest_types'
...
class PostaliApiRestSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PostaliApiRestHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PostaliApiRestHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PostaliApiRestFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PostaliApiRestUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PostaliApiRestHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PostaliApiRestHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PostaliApiRestHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => PostaliApiRestError.new(
        "#{op}_allow",
        "PostaliApiRestSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue PostaliApiRestError => err
    ...
    if fetched.is_a?(Hash)
      status = PostaliApiRestHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PostaliApiRestError.new(
        "graphql_error", "PostaliApiRestSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = PostaliApiRestSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
