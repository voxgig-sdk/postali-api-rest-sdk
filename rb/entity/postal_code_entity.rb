...
class PostalCodeEntity
  ...
  def make
    opts = @_entopts.dup
    PostalCodeEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PostaliApiRestHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [PostalCode, Hash] the current PostalCode data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PostaliApiRestHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of PostalCode fields)
  def match_get
  ...
  #
  # @param reqmatch [PostalCodeLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.PostalCode.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [PostalCode, Hash] the loaded PostalCode; raises PostaliApiRestError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PostaliApiRestHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
