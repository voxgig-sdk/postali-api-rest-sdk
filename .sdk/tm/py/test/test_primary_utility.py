...
from projectname_sdk import PostaliApiRestSDK
from projectname_sdk.core.spec import PostaliApiRestSpec
from projectname_sdk.core.result import PostaliApiRestResult
from projectname_sdk.core.response import PostaliApiRestResponse
from projectname_sdk.core.operation import PostaliApiRestOperation
from projectname_sdk.core.error import PostaliApiRestError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import PostaliApiRestBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return PostaliApiRestError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_done_basic(self):
        ...
        def subject(entry):
            ...
            except PostaliApiRestError as e:
                return None, e
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = PostaliApiRestResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = PostaliApiRestSDK.test(None, None)
        ...
        class TestInitFeature(PostaliApiRestBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(PostaliApiRestBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = PostaliApiRestSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = PostaliApiRestSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = PostaliApiRestSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = PostaliApiRestOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = PostaliApiRestSDK.test(None, setup_opts)
