...
// PostaliApiRest SDK feature test
//
...
class FtCtrl extends PostaliApiRestControl
{
...
class FtClient
{
    ...
    private PostaliApiRestUtility $utility;

    public function __construct(PostaliApiRestUtility $utility, array $options)
    {
    ...
    public function get_utility(): PostaliApiRestUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PostaliApiRestContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PostaliApiRestContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PostaliApiRestContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PostaliApiRestConfig::make_feature($name);
    ...
    private static function buildUrl(PostaliApiRestSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PostaliApiRestError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PostaliApiRestResponse($fetched);
        }
    ...
    private function fail(PostaliApiRestContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PostaliApiRestContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
