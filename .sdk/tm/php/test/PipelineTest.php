...
// PostaliApiRest SDK pipeline test
//
...
{
    private static function utility(): PostaliApiRestUtility
    {
        return new PostaliApiRestUtility();
    }
    ...
    private static function ctx(array $over = []): PostaliApiRestContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new PostaliApiRestContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new PostaliApiRestOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof PostaliApiRestError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new PostaliApiRestOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = PostaliApiRestMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new PostaliApiRestOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = PostaliApiRestMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new PostaliApiRestOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = PostaliApiRestMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PostaliApiRestSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = PostaliApiRestMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new PostaliApiRestResponse([]),
            'result' => new PostaliApiRestResult([]),
        ]);
        [, $err] = PostaliApiRestMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new PostaliApiRestSpec([]),
            'response' => null,
            'result' => new PostaliApiRestResult([]),
        ]);
        [, $err] = PostaliApiRestMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new PostaliApiRestSpec(['step' => 's']),
            'response' => new PostaliApiRestResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new PostaliApiRestResult([]),
        ]);
        [, $err] = PostaliApiRestMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new PostaliApiRestSpec(['step' => 's']),
            'response' => new PostaliApiRestResponse(self::resp(200, ['v' => 1])),
            'result' => new PostaliApiRestResult([]),
        ]);
        [, $err] = PostaliApiRestMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new PostaliApiRestSpec(['step' => 's']),
            'response' => new PostaliApiRestResponse(self::resp(200, ['v' => 2])),
            'result' => new PostaliApiRestResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        PostaliApiRestMakeResponse::call($ctx);
    ...
    {
        $preset = new PostaliApiRestResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new PostaliApiRestSpec([]),
            'response' => new PostaliApiRestResponse([]),
            'result' => new PostaliApiRestResult([]),
        ...
        [$got, $err] = PostaliApiRestMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new PostaliApiRestResult([])]);
        [, $err] = PostaliApiRestMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new PostaliApiRestSpec([]), 'result' => null]);
        [, $err] = PostaliApiRestMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PostaliApiRestSpec(['step' => 's']),
            'result' => new PostaliApiRestResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new PostaliApiRestOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PostaliApiRestMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PostaliApiRestSpec(['step' => 's']),
            'result' => new PostaliApiRestResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new PostaliApiRestOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PostaliApiRestMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PostaliApiRestResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new PostaliApiRestSpec([]),
            'result' => new PostaliApiRestResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = PostaliApiRestMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PostaliApiRestMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new PostaliApiRestError('boom', 'boom');
        $utility->fetcher = function (PostaliApiRestContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PostaliApiRestSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = PostaliApiRestMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (PostaliApiRestContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = PostaliApiRestMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(PostaliApiRestResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        PostaliApiRestMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (PostaliApiRestContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PostaliApiRestSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = PostaliApiRestMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new PostaliApiRestResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new PostaliApiRestSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = PostaliApiRestMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = PostaliApiRestMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = PostaliApiRestMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new PostaliApiRestResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, PostaliApiRestDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new PostaliApiRestResult(['ok' => false])]);
        $this->expectException(PostaliApiRestError::class);
        PostaliApiRestDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new PostaliApiRestResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', PostaliApiRestMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        PostaliApiRestMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new PostaliApiRestBaseFeature();
        $b = new PostaliApiRestBaseFeature();
        PostaliApiRestFeatureAdd::call($ctx, $a);
        PostaliApiRestFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): PostaliApiRestBaseFeature
    {
        $f = new PostaliApiRestBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        PostaliApiRestFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): PostaliApiRestContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new PostaliApiRestSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PostaliApiRestPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        PostaliApiRestPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new PostaliApiRestResponse(['headers' => null]),
            'result' => new PostaliApiRestResult([]),
        ]);
        PostaliApiRestResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        PostaliApiRestResultBody::call($ctx);
