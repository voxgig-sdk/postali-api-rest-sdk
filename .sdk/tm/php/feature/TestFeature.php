...
// PostaliApiRest SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class PostaliApiRestTestFeature extends PostaliApiRestBaseFeature
{
    ...
    public function init(PostaliApiRestContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (PostaliApiRestContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = PostaliApiRestParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (PostaliApiRestContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(PostaliApiRestContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = PostaliApiRestParam::call($ctx, $k);
