...
// PostaliApiRest SDK retry feature
...
class PostaliApiRestRetryFeature extends PostaliApiRestBaseFeature
{
    ...
    public function init(PostaliApiRestContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (PostaliApiRestContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(PostaliApiRestContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(PostaliApiRestContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
