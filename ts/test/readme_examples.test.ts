...
//     (\`new PostaliApiRestSDK(...)\` / \`PostaliApiRestSDK.test(...)\` become
//     \`PostaliApiRestSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'PostaliApiRestSDK'
...
const TEST_SEED = {"entity":{"municipality":{"test01":{"id":"test01"}},"postal_code":{"test01":{"id":"test01"}},"state":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PostaliApiRestSDK(...)\` and any
// \`PostaliApiRestSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PostaliApiRestSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PostaliApiRestSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
