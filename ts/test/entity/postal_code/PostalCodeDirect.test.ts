...
describe('PostalCodeDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new PostaliApiRestSDK({
  ...
  test('direct-load-postal_code', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-postal_code', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['POSTALI_API_REST_TEST_POSTAL_CODE_ENTID']
