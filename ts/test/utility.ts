...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.POSTALIAPIREST_TEST_LIVE ||
    'TRUE' === process.env.POSTALIAPIREST_TEST_OVERRIDE
  ) {
  ...
  m.POSTALIAPIREST_TEST_EXPLAIN = process.env.POSTALIAPIREST_TEST_EXPLAIN || m.POSTALIAPIREST_TEST_EXPLAIN
