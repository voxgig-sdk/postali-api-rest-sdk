...
describe("MunicipalityEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set POSTALI_API_REST_TEST_MUNICIPALITY_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    assert.is_not_nil(municipality_ref01_data_dt0_loaded)
...
function municipality_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("POSTALI_API_REST_TEST_MUNICIPALITY_ENTID")
