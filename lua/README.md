...
The Lua SDK for the PostaliApiRest API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Municipality(nil):load({ id = "example_id" }, nil)
...
local result, err = client:PostaliApiRest(nil):load(
  { id = "test01" }, nil
)
