# PostaliApiRest Lua SDK



The Lua SDK for the PostaliApiRest API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Municipality()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/postali-api-rest-sdk/releases)),
...
```
lua/
├── postali-api-rest_sdk.lua    -- Main SDK module
...
The main module (`postali-api-rest_sdk`) exports the SDK constructor
