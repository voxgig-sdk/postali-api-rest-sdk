...
local PostalCodeEntity = {}
PostalCodeEntity.__index = PostalCodeEntity


function PostalCodeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PostalCodeEntity)
  self._name = "postal_code"
  self._client = client
...
function PostalCodeEntity:get_name()
  return self._name
end


function PostalCodeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PostalCodeEntity.new(self._client, opts)
end
...
function PostalCodeEntity:mark_deleted()
  self._deleted = true
end


function PostalCodeEntity:deleted()
  return true == self._deleted
end


function PostalCodeEntity:data_set(args)
  if args ~= nil then
...
function PostalCodeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PostalCodeEntity:match_set(args)
  if args ~= nil then
...
function PostalCodeEntity:match_get()
...
function PostalCodeEntity:stream(action, args, callopts)
...
---@param reqmatch PostalCodeLoadMatch
---@param ctrl? table
---@return PostalCode
---@return string? err
function PostalCodeEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PostalCodeEntity:_run_op(ctx, post_done)
  local utility = self._utility
