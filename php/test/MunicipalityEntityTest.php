...
class MunicipalityEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set POSTALI_API_REST_TEST_MUNICIPALITY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $this->assertNotNull($municipality_ref01_data_dt0_loaded);
...
function municipality_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["municipality01", "municipality02", "municipality03", "municipio01", "municipio02", "municipio03"] as $k) {
    ...
    $entid_env_raw = getenv("POSTALI_API_REST_TEST_MUNICIPALITY_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["POSTALI_API_REST_TEST_MUNICIPALITY_ENTID"]);
