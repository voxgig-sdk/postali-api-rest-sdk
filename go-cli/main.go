// Command PostaliApiRest-cli is a CLI + REPL for the PostaliApiRest SDK,
...
const prompt = "postali-api-rest"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "municipality postal_code state"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      POSTALI_API_REST_APIKEY (api key), POSTALI_API_REST_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: POSTALI_API_REST_APIKEY carries the API key and
	// POSTALI_API_REST_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("POSTALI_API_REST_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("POSTALI_API_REST_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPostaliApiRestSDK(opts)
